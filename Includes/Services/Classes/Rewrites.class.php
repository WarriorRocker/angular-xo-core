...
class XoServiceRewrites
{
	...
	protected function FormatQueryVars($vars) {
		...
		foreach ( $public_query_vars as $wp_var) {
			if (isset($vars[$wp_var])) {
				$query_vars[$wp_var] = $vars[$wp_var];
			}

			if (!empty($query_vars[$wp_var])) {
				if (!is_array($query_vars[$wp_var])) {
					$query_vars[$wp_var] = (string)$query_vars[$wp_var];
				} else {
					foreach ($query_vars[$wp_var] as $vkey => $v) {
						if (is_scalar($v)) {
							$query_vars[$wp_var][$vkey] = (string)$v;
						}
					}
				}

				if (isset($post_type_query_vars[$wp_var])) {
					$query_vars['post_type'] = $post_type_query_vars[$wp_var];
					$query_vars['name'] = $query_vars[$wp_var];
				}
			}
		}
